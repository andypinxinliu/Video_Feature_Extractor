...
class I3D_BackBone(nn.Module):
    def __init__(self, model_path='model/rgb_imagenet.pt', final_endpoint='Mixed_5c', name='inception_i3d', in_channels=3,
                 freeze_bn=True, freeze_bn_affine=True, window_size=64):
    ...
    def load_pretrained_weight(self, model_path='model/rgb_imagenet.pt'):
        print("Loading I3D pretrained weight from {}...".format(model_path))
        self._model.load_state_dict(torch.load(model_path), strict=False)

    def forward(self, x):
        feat = self._model.extract_features(x)['Mixed_5c']
        feat = self.avg_pool_5c(feat).squeeze(3).squeeze(3)
        return feat
