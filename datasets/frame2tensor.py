...
dataset = 'charade' # or charades

if dataset == 'multithumos':
    splits = ['training','testing']
    frame_folder_path = 'data/thumos14/' # path to rgb frame folder
    tensor_path = 'data/thumos14_tensors/' # path to image tensor folder
else:
    splits = ['']
...
for split in splits:
    ...
    for vid in tqdm(vid_list):
        ...
        new_path = os.path.join(tensor_path, split)
        if not os.path.exists(new_path):
            os.makedirs(new_path)
        torch.save(img_tensor, os.path.join(new_path, vid))
